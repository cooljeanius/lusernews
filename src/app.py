...
def latest(request, start=None):
    ...
    return util.render("latest.pat", news=news, user=g.user, start=start, next=next)
...
def newspage(request, news_id):
    ...
    if not news:
        return util.render("error.pat", user=g.user, error="the news does not exist")
...
def userpage(request, username):
    ...
    if not user:
        return util.render("error.pat", user=g.user, error="the user does not exist")
...
def editnews(request, news_id):
    auth_user(request.cookies.get("auth"))
    if not g.user:
        return util.render("error.pat", user=g.user, error="you have to login first")
...
def saved(request, start=None):
    ...
    return util.render("saved.pat", news=news, user=g.user, start=start, next=next)
